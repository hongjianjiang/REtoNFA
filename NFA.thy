...
definition NFA_accept :: "('a, 'b) NFA_rec \<Rightarrow> 'b list \<Rightarrow> bool" where
  "NFA_accept \<A> w = (if \<Delta>' \<A> \<noteq> {} then
                  (\<exists> q \<in> (\<I> \<A>). \<exists> q' \<in> (\<F> \<A>). \<exists> (q,q'') \<in> (\<Delta>' \<A>).LTS_is_reachable (\<Delta> \<A>) q'' w q')  
                        else (\<exists> q \<in> (\<I> \<A>). \<exists> q' \<in> (\<F> \<A>).
